<template>
    <main id="Product-page">
        <form class="flex items-center ml-12 mt-20 relative">
            ...
            <div class="flex items-center relative w-full">
                ...
                <div class="relative">
                    ...
                    >
                        <ul
                            class="h-48 py-2 overflow-y-auto text-left text-gray-700"
                        >
                            <li>
                                <a
                                    href="#"
        ...
        <div
            v-if="isLoading"
            class="loading-message items-center py-8 px-8 ms-2 me-6 text-base font-medium text-gray-500"
        >
...
import DeleteProdModal from "./DeleteProdModal.vue";
import EditProdModal from "./EditProdModal.vue";
import InfoProdModal from "./InfoProdModal.vue";
import DeleteAllProdModal from "./DeleteAllProdModal.vue";
...
export default {
        ...
        filteredProducts() {
            ...
            if (!query && !this.selectedCategory) {
                ...
                return this.products.filter(
                    (product) => product.category === this.selectedCategory.name
                );
            } else if (query && !this.selectedCategory) {
    ...
    methods: {
        async updateProductsCategories() {
            try {
                ...
                for (let product of this.products) {
                    await axios
                        .get(
                            `http://127.0.0.1:8000/api/categories/${product.category_id}`
                        )
                        .then((response) => {
                            product.category = response.data.category.name;
                        })
                        .catch((error) => {
                            console.error(
                                "There was an error assigning category name to a product:",
                                error
                            );
                        });
                }
                ...
                for (let category of this.categories) {
                    await axios
                        .get(`/api/categories/${category.id}/products/count`) // Effectuer une requête GET
                        .then((response) => {
                ...
                this.isLoading = false; // Désactive l'indicateur de chargement une fois les données récupérées
            }
        },
            ...
            if (this.editCategoryName.trim() !== "") {
                try {
                    await axios
                        ...
                        .catch((error) => {
                            console.error(
                                "There was an error editing the category:",
                                error
                            );
                        });
                } catch (error) {
        ...
        getImageUrl(imagePath) {
            if (!imagePath) {
                // Retourner une image par défaut si aucune image n'est fournie
        ...
        // Calculer les numéros de page précédents et suivants
        nextPage() {
